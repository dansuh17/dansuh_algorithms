...
class NewsClustering {
  ...
  public int solution(String str1, String str2) {
    int[] set1 = makeSet(str1);
    int[] set2 = makeSet(str2);

    int[] unionSet = union(set1, set2);
    int[] intersectionSet = intersetcion(set1, set2);

    long unionSum = IntStream.of(unionSet).sum();
    long interSum = IntStream.of(intersectionSet).sum();

    if (unionSum == interSum) {
      return 65536;
    }

    double d = 65536.0D / (double)unionSum;
    return (int)(d * interSum);
  }

  public int[] union(int[] set1, int[] set2) {
    int len = set1.length;
    int[] unionSet = new int[len];
    for (int i = 0; i < len; i++) {
      unionSet[i] = Math.max(set1[i], set2[i]);
    }

    return unionSet;
  }

  public int[] intersetcion(int[] set1, int[] set2) {
    int len = set1.length;
    int[] intersectionSet = new int[len];
    for (int i = 0; i < len; i++) {
      intersectionSet[i] = Math.min(set1[i], set2[i]);
    }

    return intersectionSet;
  }

  public int[] makeSet(String str) {
    str = str.toLowerCase();  // convert to lowercase

    char[] charBuf = new char[2];
    int[] pairSet = new int[NUM_ALPHABETS * NUM_ALPHABETS + 1];
    charBuf[0] = str.charAt(0);

    for (int i = 1; i < str.length(); i++) {
      char c = str.charAt(i);
      charBuf[1] = c;

      // both should be alphabet
      if (isAlpha(charBuf[0]) && isAlpha(charBuf[1])) {
        pairSet[toPairIndex(charBuf[0], charBuf[1])]++;
      }
  ...
  public int toPairIndex(char c1, char c2) {
